function stamp=find_adaptive_event(data,th,id_type,th_type,reject)
% this function find points bigger than pre defined threshold
...
r_ind=[find(data(index)>reject*th(index));find(data(index)<(-reject*th(index)))];
if ~isempty(r_ind)
     index(r_ind)=[];
end
...
discard=zeros(numel(stamp),1);
for i=1:numel(stamp)
    try
        temp=data(stamp(i)-255:stamp(i)+256);
        if any(temp>reject.*th(stamp(i)))
            discard(i)=1;
        elseif any(temp<-reject.*th(stamp(i)))
            discard(i)=1;
        end
    catch
    end
